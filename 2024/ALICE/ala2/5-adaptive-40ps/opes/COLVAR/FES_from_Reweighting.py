...
parser.add_argument('--bin',dest='grid_bin',type=str,default="250,250",help='number of bins for the grid')
# blocks
...
# NB: the following np.exp cannot be easily made 100% numerically safe, but using np.expm1 makes it more robust
  ...
  ### Mingyuan: Save these errors for plotting
  np.savetxt('err_fes_{colvar}_{blocks_num}'.format(colvar=args_cv,blocks_num=blocks_num),fes_err)
  print(fes_err)
  with open('log{colvar}'.format(colvar=args_cv),'a+') as f:
      f.writelines(str(np.average(fes_err))+'\n')
